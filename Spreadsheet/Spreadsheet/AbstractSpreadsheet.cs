...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// 
        /// Given a starting cell, determines the cells that are in relation to this starting cell.
        /// An alorithm similar to recursive depth-first search is used, where the starting cell's dependents 
        /// are traversed down the line of cell dependencies until there is no dependency for the last cell.
        /// Then, a 'backtrack' occurs, where the previous cell is returned to (due to recursion) and the same
        /// algorithm is performed for any remaining cell dependents of that cell. Each cell that is traversed is 
        /// added to a 'visited' list so that cells that have already been checked will not be checked again. The
        /// backtracking process occurs until there are no more recursive calls and all cells to be recalculated 
        /// are added to the 'changed' list.
        /// </summary>
        /// <exception cref="CircularException"> Thrown when a circular dependency occurs. </exception>
        private void Visit(string start, string name, ISet<string> visited, LinkedList<string> changed)
        {
            visited.Add(name);
            foreach (string n in GetDirectDependents(name))
            {
                // Dependents of the starting cell may not have the starting cell as one of their dependents.
                if (n.Equals(start))
                {
                    throw new CircularException();
                }
                // If the cell has not yet been visited, all of its dependents are checked.
                else if (!visited.Contains(n))
                {
                    Visit(start, n, visited, changed);
                }
            }
            // The cells stored in the complete 'changed' list will be in order of cells to be recalculated.
